...
while(<STDIN>) {
  ...
  if(IsNormalWavFile($wav_rspecifier) == 1) {
    ...
    $wav_rspecifier = "ffmpeg -nostats -loglevel error -f wav $wav_rspecifier  -ac 1 -ar 8000 -sample_fmt s16  $codecList[$index]  pipe:1| ffmpeg -nostats -loglevel error -f $codec_parts[1] -i pipe:0 -ar 16000 -f wav pipe:1 |";
    ...
    $wav_rspecifier = "$wav_rspecifier ffmpeg -nostats -loglevel error -f wav -i pipe:0 -ac 1 -ar 8000 -sample_fmt s16 $codecList[$index]  pipe:1| ffmpeg -nostats -loglevel error -y -f $codec_parts[1] -i pipe:0 -f wav -ar 8000 pipe:1 |";
  }
  print "$wavid $wav_rspecifier\n";
}
