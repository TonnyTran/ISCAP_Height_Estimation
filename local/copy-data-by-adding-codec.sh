...
if [ ! -z $step01 ]; then
  copy_data_dir.sh --spk-prefix codec1.0v- --utt-prefix codec1.0v- \
  $source_data $tmpdir 
fi
...
if [ ! -z $step02 ]; then
  ...
  cat $tmpdir/segments | ./local/rewrite-wav-scp.pl $tmpdir/wav.scp > $target_data/new_wav.scp
fi
...
if [ ! -z $step03 ]; then
  ...
  ./local/add-codec-with-ffmpeg.pl  $codec_list > $target_data/wav.scp
